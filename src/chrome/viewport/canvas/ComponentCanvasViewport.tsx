...
import { TbBoxModel2, TbX } from "react-icons/tb";
...
import { useCanvasModeBody } from "./useCanvasModeBody";
...
export function ComponentCanvasViewport({ className = "" }: Props) {
  ...
  // Tag body + inject style sheet on mount; lift clipping/transform on the
  // #viewport ancestor chain; restore on unmount. MUST stay the first effect so
  // its mount/cleanup ordering relative to the pan/zoom + isolation effects below
  // is preserved.
  useCanvasModeBody();
  ...
  return (
    ...
    >
      ...
      <CanvasFloatingToolbar
        isolated={!!canvasIsolate}
        componentCount={allCardIds.length}
        onCreateComponent={createComponent}
        onArrange={arrangeCards}
