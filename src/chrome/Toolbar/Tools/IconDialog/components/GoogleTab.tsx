      ...
      <div className="shrink-0 border-b border-border bg-background p-3">
        ...
        <div className="mb-3">
          <div className="mb-1.5 flex items-center justify-between text-xs font-medium text-foreground">
            Category
            <div className="text-xs text-muted-foreground">{d.filteredIcons.length} icon{d.filteredIcons.length !== 1 ? "s" : ""}</div>
          </div>
          <div className="scrollbar-light flex gap-1.5 overflow-x-auto py-1" role="group" aria-label="Icon categories">
            {GOOGLE_CATEGORIES.map(cat => (
              <button
                key={cat.id}
                className={`shrink-0 rounded-lg px-2 py-1 text-xs font-medium transition-colors ${d.category === cat.id ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}
          ...
          <div className="scrollbar-hide flex items-center gap-3">
            <div className="flex shrink-0 gap-1.5" role="group" aria-label="Icon style">
              {ICON_STYLES.map(style => (
                <button
                  key={style.id}
                  className={`shrink-0 rounded-lg px-2 py-1 text-xs font-medium transition-colors ${d.iconStyle === style.id ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}
...
function SliderControl({ id, label, min, max, step, value, onChange }: {
  ...
  return (
    <div className="flex shrink-0 items-center gap-2 border-l border-border pl-3 first:border-l-0 first:pl-0">
      ...
      <input
        ...
        className="slider h-2 w-20 cursor-pointer appearance-none rounded-lg bg-muted"
      />
