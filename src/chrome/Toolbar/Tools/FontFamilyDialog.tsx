  ...
  const FontItem = ({ index, style }: { index: number; style: React.CSSProperties }) => {
    ...
    return (
      <div style={style}>
        <button
          id={`fontFamily-${font}`}
          className={`flex w-full cursor-pointer flex-row truncate whitespace-nowrap rounded-lg p-1 text-xs text-muted-foreground transition-colors hover:bg-muted ${
