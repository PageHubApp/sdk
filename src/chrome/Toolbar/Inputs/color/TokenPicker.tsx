...
export function TokenPicker({ value, onChange, onClose, variant = "inline" }: TokenPickerProps) {
  ...
  useEffect(() => {
    try {
      ...
      const pallet = (node?.data?.props as any)?.pallet || [];
      setPalette(pallet.filter((p: any) => p?.name && p?.color));
    } catch {}
  }, [query]);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1">
        ...
        <button onClick={() => setShowCreateDialog(true)} className={`${swatchSize} flex items-center justify-center rounded-md border-2 border-dashed border-border text-muted-foreground transition-colors hover:border-foreground hover:text-foreground`} title="New token">
          <TbPlus className="size-3" />
        </button>
        <button onClick={() => setShowAll(prev => !prev)} className="ml-auto flex shrink-0 items-center text-muted-foreground hover:text-foreground" title={showAll ? "Less" : "All tokens"}>
...
function Swatch({ color, selected, onPick, onHover, size = "size-6" }: { color: NamedColor; selected: boolean; onPick: (name: string) => void; onHover?: (name: string | null) => void; size?: string }) {
  ...
  return (
    ...
    >
      <div
        className={`${size} flex items-center justify-center rounded-md border-2 transition-transform hover:scale-110 ${selected ? "border-primary ring-1 ring-primary/30" : "border-border"}`}
