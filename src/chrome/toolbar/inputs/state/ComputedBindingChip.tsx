/**
 * ComputedBindingChip — chip + popover for one entry in
 * `Container.computedStateBindings`. Click → opens an editor panel that
 * mutates the binding's `compute` shape (all-truthy / first-truthy / join /
 * variant-match / variant-axis-availability). Variant types swap their input
 * fields in below the type selector.
 */
...
export const COMPUTE_TYPES: Array<{
  value: ComputedStateCompute["type"];
  label: string;
  description: string;
}> = [
  {
    ...
    label: "All truthy",
    description:
      "Output 'on' when every input key has a non-empty value. Use as an Add-to-Cart gate.",
  },
  {
    value: "first-truthy",
    label: "First truthy",
  ...
  },
  {
    ...
    label: "Variant match",
    description:
      "Match axis selections against a variant map JSON and emit the matched variant. Use on a PDP wrapper.",
  },
  {
    value: "variant-axis-availability",
    label: "Axis availability",
    description:
      "Emit a CSV of axis values that have no in-stock variant given the currently selected other axes.",
  },
];
    ...
    >
      <div className="flex flex-col gap-3">
        ...
        {c.type === "variant-axis-availability" && (
          <>
            <label className="flex flex-col gap-0.5">
              <span className="text-base-content text-[11px] font-medium">
                Variant map (state key or JSON)
              </span>
              <input
                type="text"
...
export function ComputedBindingChipRow({
  binding,
  onChange,
  onRemove,
}: {
  ...
  const typeLabel =
    COMPUTE_TYPES.find(t => t.value === binding.compute.type)?.label ?? binding.compute.type;
