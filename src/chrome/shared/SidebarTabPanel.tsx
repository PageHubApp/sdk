...
export function SidebarTabPanel({
  ...
  return (
    <div
      className={twMerge(
        "divide-sidebar-border flex min-h-0 flex-1 flex-col divide-y overflow-hidden",
        className
      )}
    >
      <div className="shrink-0">{tabList}</div>
      <div className={twMerge("flex min-h-0 flex-1 flex-col overflow-hidden", bodyClassName)}>
        {children}
      </div>
    </div>
  );
