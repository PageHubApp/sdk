...
function init(){
  ...
  // 3. DOM bindings
  attachStateBindings();
  // 4. Actions — now an Alpine.directive('actions'); registered above. Alpine.start
  //    walks the DOM after init() and applies it. No call needed here.
  // 5. Forms — same: Alpine.directive('form') above handles it.
  // 6. Connector refetch
...
function rebind(root){
  ...
  // data-ph-actions + data-ph-form are now Alpine directives — Alpine's
  // MutationObserver auto-picks them up on element insertion, so no manual
  // rebind needed for those two attribute names.
  void root;
  attachStateBindings();
  attachCartItems();
  attachConnectorRefetch();
